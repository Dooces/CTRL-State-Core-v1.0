...
CTRL-State Core v1.1 — Predictive Control State Mapper
...
• State: capacity, decision_threshold, precision allocations, prior↔likelihood balance, planning horizon, residual/engagement notes.  
• Prescriptions: exactly 3 levers from a fixed list (see constraints below).  
• Behavior deltas: exactly 3 directional predictions from a fixed list. These are **expected changes over the next short horizon (e.g., 2–4 weeks) given the three levers**.  
...
decision_threshold: low | normal | high
  Meaning: gate to commit/allow content to influence action (response threshold).

policy_precision: low | normal | high
  Meaning: action-selection sharpness (inverse temperature/decisiveness).

sensory_precision: low | normal | high
  Meaning: evidence weighting for external input (likelihood weight/credibility).

priors_vs_likelihoods: priors>likelihoods | balanced | likelihoods>priors
  Meaning: top-down vs bottom-up weighting in inference.

planning_horizon: short | normal | long
  Meaning: temporal look-ahead window for plans.
...
Fixed lever set (pick exactly 3; obey constraints)
...
Lever selection constraints (determinism/consistency)
• Never pick both increase_* and decrease_* for the same dimension.  
• Never pick both block_behavioral_routine and allow_behavioral_routine.  
• If sensory_precision = low, include **either** increase_sensory_precision **or** decrease_prior_precision (prefer the former if both seem justified).  
• If decision_threshold = high with prominent avoidance/rumination, prefer decrease_decision_threshold; if impulsivity/risk-taking dominates with low θ, prefer increase_decision_threshold.  
• If planning_horizon = short with over-narrow goals, prefer increase_planning_horizon; if paralysis from over-projection, prefer decrease_planning_horizon.  
• realign_capacity_estimate is allowed only when capacity ≠ normal **or** symptoms indicate sleep/circadian/metabolic disruption.
...
Behavior-delta gating (mandatory)
Use “stabilize” (i.e., no expected directional change over the horizon) when symptom evidence for that metric is weak, mixed, or implies small effects. Do **not** guess a direction when cues conflict.
...
You are a mapper. Read symptom-only text. Output ONLY a JSON object matching the CTRL-State Core v1.1 schema below. Do not add commentary.

Schema (verbatim): [paste the JSON schema above]

Symptoms: [PASTE CASE TEXT HERE]
...
• Mandatory “stabilize” gate prevents forced directional guesses when evidence is weak or mixed.

How to run your “fresh model” test
Give the fresh model the prompt template + schema. Feed symptom-only vignettes. Collect the JSON and check:
• Consistency: same lever set and delta directions across runs/models?  
• Face validity: do deltas match expected change directions given the levers?  
• Transport: try varied sources/societies; allow only intercept-style calibration (e.g., none).
